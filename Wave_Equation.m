...
% Create initial condition u_y(x, 0) = g(x):

BC2_y = create_BC(2, Ny, 1, My);
BC2_y_val = g(x);

% Define linear left-hand side operator d^2/dy^2 - c^2 * d^2/dx^2:

D = create_operator(2, eye(Nx), 0, My^2, 0) - ...
    c^2 * create_operator(2, Mx^2, 0, eye(Ny), 0);
